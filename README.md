# Event-V
Event-V is an experimental library and set of tools for modelling software systems. It is based on the methodology pioneered by [Event-B](https://en.wikipedia.org/wiki/B-Method#Event-B), in which simple, abstract models are incrementally **refined** until they are detailed enough to be executed as code. The framework requires each step to prove that it is a valid implementation of the abstract model described in the previous step. In this way, **a formally verified program can be produced step by step**, each time only adding a small amount of complexity.
